		...
		bool evaluateComplexScopedLocalFunctions )
{
	...
	if (loadOkay)
	{
		if(verbose) cout<<"\tRead was successful... begin parsing..."<<endl<<endl;
		...
		if ((RAZI_DEBUG) & READ_FILE) cout<<"\n =========================================================================================\n\n";
		if(!verbose) cout<<"-";
		else {cout<<"\nReading parameter list..."<<endl; mypause(5000);}
		...
		if ((RAZI_DEBUG) & READ_FILE) {cout<<"\n =========================================================================================\n\n"; mypause(-1);}
		if(!verbose) cout<<"-";
		else {cout<<"\nReading list of MoleculeTypes..."<<endl; mypause(5000);}
		...
		if ((RAZI_DEBUG) & READ_FILE){ cout<<"\n =========================================================================================\n\n";  mypause(-1);}
		if(!verbose) cout<<"-";
		else {cout<<"\nReading list of Species..."<<endl; mypause(5000);}
		...
		if(!verbose) cout<<"-";
		else {cout<<"\nReading list of Observables ..."<<endl; mypause(5000);}
		...
		if(!verbose) cout<<"-";
		else if(pListOfFunctions) {cout<<"\n\tReading list of Functions..."<<endl; mypause(5000);}
		if(pListOfFunctions)
		{
		...
		if(!verbose) cout<<"-";
		else {cout<<"\nReading list of Reaction Rules..."<<endl; mypause(5000);}
		...
		bool verbose)
{
	...
	try {
		...
		for ( pMoTypeEl = pListOfMoleculeTypes->FirstChildElement("MoleculeType"); pMoTypeEl != 0; pMoTypeEl = pMoTypeEl->NextSiblingElement("MoleculeType"))
		{
			...
			if(verbose) cout<<"\n\tReading and Creating MoleculeType: "+typeName+"(";
					...
					if(pListOfAllowedStates)
					{
						...
						if(stringComponentNameCount>0) {
							...
							for ( pAlStates = pListOfAllowedStates->FirstChildElement("AllowedState"); pAlStates != 0; pAlStates = pAlStates->NextSiblingElement("AllowedState"))
							{
								...
								//razi commented, later uncomment cout<<"\nadding allowed state: "<<typeName+"_"+compName+"_"+aState<<" to be "<<allowedStateCount;
								if(verbose || (((RAZI_DEBUG) & CREATE_MOLECULE))) cout<<"\n\tadding allowed state: "<<typeName+"_"+compName+"_"+aState<<" to be "<<allowedStateCount;
	...
	try {
		...
		for ( pSpec = pListOfSpecies->FirstChildElement("Species"); pSpec != 0; pSpec = pSpec->NextSiblingElement("Species"))
		{
			...
			*/
			...
			if(verbose) cout<<"\n\tCreating "<<specCountInteger<<" instances of the Species: "<<speciesName<<endl;
		...
		for ( pRxnRule = pListOfReactionRules->FirstChildElement("ReactionRule"); pRxnRule != 0; pRxnRule = pRxnRule->NextSiblingElement("ReactionRule"))
		{
			...
			for( unsigned int p=0; p<permutations.size(); p++)
			{
				...
				for ( pReactant = pListOfReactantPatterns->FirstChildElement("ReactantPattern"); pReactant != 0; pReactant = pReactant->NextSiblingElement("ReactantPattern"))
				{
					...
					if(verbose) cout<<"\n\tReading Reactant Pattern: "<<reactantName<<endl;
...
//if (((RAZI_DEBUG) & CHECK_LBG) && show_flag){tm->printDetails(); mypause(-1); }
				...
				for ( pAdd = pListOfOperations->FirstChildElement("Add");pAdd != 0;  pAdd = pAdd->NextSiblingElement("Add") )
				{
					...
					if(verbose) cout<<"\t=>ified addition of product: "+id+"."<<endl;
...
//cout<<"NFInput: Show components after reading;\n"; mypause(100); iii=0; for(map <string, component>::iterator it = comps.begin(); it != comps.end(); it++, iii++){ cout<<"component["<< iii<<"] A:"<< it->first<< "  B:"<< it->second.name<<"    ";}
				...
				for ( pStateChange = pListOfOperations->FirstChildElement("StateChange"); pStateChange != 0; pStateChange = pStateChange->NextSiblingElement("StateChange"))
				{
					...
					if(finalState=="PLUS") {
						...
						if(verbose) {
							cout<<"\t=>ified decrement state of site: "+c->t->getMoleculeTypeName()+"("+c->symPermutationName;
							cout<<") to new state value: oldStateValue-1"<<endl;
						}
						...
						pAddBond != 0;  pAddBond = pAddBond->NextSiblingElement("AddBond") )
				{
					...
					else
					{
						...
						if (verbose)
						{
							cout << "\t=>ified binding of site: " << c1->t->getMoleculeTypeName()
				...
				if(!pRateLaw->Attribute("id") || !pRateLaw->Attribute("type")) {
					...
					if(verbose) cout<<"\n\tRate Law for Reaction is: "<<rateLawType<<endl;
					if(rateLawType=="Ele")
					{
						...
						if(!pListOfRateConstants) {
							cerr<<"Elementry Rate Law definition for "<<rxnName<<" does not have listOfRateConstants specified!  Quitting"<<endl;
							return false;
						}
						TiXmlElement *pRateConstant = pListOfRateConstants->FirstChildElement("RateConstant");
						if(!pRateConstant) {
							cerr<<"Elementry Rate Law definition for "<<rxnName<<" does not have RateConstants specified!  Quitting"<<endl;
							return false;
						} else {
							...
							if(!pRateConstant->Attribute("value")) {
								cerr<<"Elementry Rate Law definition for "<<rxnName<<" does not have a valid RateConstant value!  Quitting"<<endl;
								return false;
							} else {
		...
		int &suggestedTraversalLimit)
{
	...
	try {
		...
		for ( pObs = pListOfObservables->FirstChildElement("Observable"); pObs != 0; pObs = pObs->NextSiblingElement("Observable"))
		{
			...
			if(verbose) cout<<"\n\tCreating Observable: '"<<observableName<<"' of type: '"<<observableType<<"'"<<endl;
			...
			if(observableType.compare("Molecules")==0)
			{
				...
				//razi commented, later uncomment cout<<"by default, so this type of observable requires complex bookkeeping to be turned on"<<endl;
				//razi commented, later uncomment cout<<"which makes NFsim run slower.  Be sure that you need this observable!\n"<<endl;
						...
						if(pComp->Attribute("state")) {
							...
							if(compStateValue!="*" && compStateValue!="?") {
								if(allowedStates.find(molName+"_"+compName+"_"+compStateValue)==allowedStates.end()) {
									...
									cerr<<"invalid state! The state you gave was: '"<<compStateValue<<"'.  Quitting now."<<endl;
									return NULL; //Razi changed for compile compatibility, was  return false;
								} else {
									//State is a valid allowed state, so push it onto our list
...
//		}
		...
		if(verbose) { cout<<"\treadPattern=> Final processed pattern: "<<finalTemplate->getPatternString()<<endl; }
