...
void MoleculeType::prepareForSimulation()
{
	...
	Molecule *mol;
  	for( int m=0; m<mList->size(); m++ )
  	{
		...
		for(rxnIter = reactions.begin(), r=0; rxnIter != reactions.end(); rxnIter++, r++ )
		{
//cout<<"MoleculeType::prepareForSimulation by adding the molecule to reaction:"<< (*rxnIter)->getName()<<endl;
			(*rxnIter)->tryToAdd(mol, reactionPositions.at(r));
//cout<<"MoleculeType::prepareForSimulation: finished adding the molecule to reaction:"<< (*rxnIter)->getName()<<endl;
  		}
	}
}
...
void MoleculeType::printDetails() const
{
	...
	cout<<"   -has "<< reactions.size() <<" reactions. ";
	//	cout<<"        of which "<< indexOfDORrxns.size() <<" are DOR rxns. "<<endl;
	for(unsigned int tii=0; tii < reactions.size(); tii++) 		cout<< reactions.at(tii)->getName()<<", ";  cout<<endl;


	cout<<"   -has "<< molObs.size() <<" molecules observables ";
	for(unsigned int tii=0; tii < molObs.size(); tii++)		cout<< molObs.at(tii)->getName()<<", "; 	cout<<endl;
}
