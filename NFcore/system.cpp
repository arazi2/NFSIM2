...
void System::prepareForSimulation()
{
	...
	if(verbose && (RAZI_DEBUG & CREATE_FUNC)) {cout<<"System: Preparing "<< globalFunctions.size()<<" global functions ...";  mypause(WTIME); };
  	for( functionIter = globalFunctions.begin(); functionIter != globalFunctions.end(); functionIter++ )
  		(*functionIter)->prepareForSimulation(this);

	if(verbose && (RAZI_DEBUG & CREATE_FUNC)) {cout<<"System: Preparing  "<< localFunctions.size() <<" local functions ...";  mypause(WTIME); };
  	for( int f=0; f<localFunctions.size(); f++)
  		localFunctions.at(f)->prepareForSimulation(this);

  	//cout<<"here 2..."<<endl;

  	if(verbose && (RAZI_DEBUG & CREATE_FUNC)) {cout<<"System: Preparing "<<compositeFunctions.size() <<" composite functions ...";  mypause(WTIME); };
  	for( int f=0; f<compositeFunctions.size(); f++)
  		compositeFunctions.at(f)->prepareForSimulation(this);

//cout<<"here 3..."<<endl;
  	if(verbose && (RAZI_DEBUG & CREATE_FUNC)) {this->printAllFunctions(); mypause(WTIME);}

  	// now we prepare all reactions
	if(verbose && (RAZI_DEBUG & CREATE_REACTION)) {cout<<"Press a key or wait a few seconds to prepare "<<allReactions.size()<<" reactions ...";  mypause(WTIME); }
...
//cout<<"here 5, reactions prepared..."<<endl;  mypause(-1);
	...
	if(verbose && (RAZI_DEBUG & CREATE_REACTION)) {cout<<"Press a key or wait a few seconds to add setup all local functions for molecule-types ...";  mypause(WTIME); };
	if(this->localFunctions.size()>0) {
	  	for( molTypeIter = allMoleculeTypes.begin(); molTypeIter != allMoleculeTypes.end(); molTypeIter++ )
	  		(*molTypeIter)->setUpLocalFunctionListForMolecules();
	}

//cout<<"here 6..."<<endl; mypause(-1);


  	//prep each molecule type for the simulation
	if(verbose && (RAZI_DEBUG & CREATE_MOLECULE)) {cout<<"Press a key or wait a few seconds to prepare molecule types ...";  mypause(WTIME); };
  	for( molTypeIter = allMoleculeTypes.begin(); molTypeIter != allMoleculeTypes.end(); molTypeIter++ )
  		(*molTypeIter)->prepareForSimulation();

//cout<<"here 7..."<<endl;  mypause(-1);
	...
	if(verbose && (RAZI_DEBUG &SHOW_SIM)) {cout<<"Press a key or wait a few seconds to update "<< speciesObservables.size() <<"species observable count based on "<< allComplexes.getComplexCount() <<" complexes ...";  mypause(WTIME); };
  	int match = 0;
...
// 	}
	...
	//this->selector = new LogClassSelector(allReactions);
  	...
  	if(verbose && (RAZI_DEBUG &SHOW_SIM))  {cout<<"Press a key or wait a few seconds to evaluate local ...";  mypause(WTIME); };
	this->evaluateAllLocalFunctions();

	if(verbose && (RAZI_DEBUG &SHOW_SIM))  {cout<<"Press a key or wait a few seconds to recompute A ...";  mypause(WTIME); };
  	recompute_A_tot();
...
double System::getNextRxn()
{
	...
	if	(RAZI_DEBUG & RUN_REACTIONS){
		...
		cout<<"System selects the next reaction. Reaction id [1]:"<< x <<"  [2]:" << y <<endl; mypause(0);
		return y;
	}else{
...
{
	return sim(duration,sampleTimes,verbose);
}
